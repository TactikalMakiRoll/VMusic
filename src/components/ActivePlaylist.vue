<template>
  <aside
    class="flex w-screen shrink-0 flex-col bg-[rgba(19,19,19,1)] py-6 px-6 backdrop-opacity-100 sm:w-auto md:py-11 md:pl-8 md:pr-11"
  >
    <button
      @click="$emit('closePlaylist')"
      class="w-10 cursor-pointer self-end hover:brightness-200"
    >
      <img
    ...
    >
      <div class="mt-auto w-full rounded-xl bg-black/60 px-8 py-8 text-center backdrop-blur-xl">
        ...
        <div class="relative">
          ...
          ></div>
          <div
            class="absolute top-0 left-0 h-1 w-6/12 rounded-xl bg-white"
            id="progress"
          ></div>
        </div>
      </div>
